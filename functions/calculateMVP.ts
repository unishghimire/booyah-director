...
Deno.serve(async (req) => {
  try {
    ...
    for (const evt of killEvents) {
      ...
      if (!killMap[pid]) killMap[pid] = { name: evt.data.killer_name, team: evt.data.killer_team_name, kills: 0, player_id: pid };
      killMap[pid].kills++;
    }

    const sorted = Object.values(killMap).sort((a, b) => b.kills - a.kills);
    if (sorted.length === 0) return new Response(JSON.stringify({ mvp: null, tied: false, message: 'No kills recorded' }), { headers: { 'Content-Type': 'application/json' } });

    const maxKills = sorted[0].kills;
    const topPlayers = sorted.filter(p => p.kills === maxKills);

    return new Response(JSON.stringify({ mvp: topPlayers[0], tied: topPlayers.length > 1, tied_players: topPlayers, max_kills: maxKills }), { headers: { 'Content-Type': 'application/json' } });
  } catch (err: any) {
