...
Deno.serve(async (req) => {
  try {
    ...
    const team = await base44.asServiceRole.entities.Team.get(player.data.team_id);
    ...
    let pointsPerKill = 1;
    if (player.data.tournament_id) {
      const tournament = await base44.asServiceRole.entities.Tournament.get(player.data.tournament_id);
      pointsPerKill = tournament?.data?.points_per_kill || 1;
    }
    ...
    const standings = await base44.asServiceRole.entities.MatchStanding.filter({ match_id, team_id: team.id });
    if (standings.length > 0) {
      const standing = standings[0];
      await base44.asServiceRole.entities.MatchStanding.update(standing.id, {
    ...
    if (overlayStates.length > 0) {
      await base44.asServiceRole.entities.OverlayState.update(overlayStates[0].id, { last_updated_at: new Date().toISOString() });
    }
