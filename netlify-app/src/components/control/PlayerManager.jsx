  ...
  return (
    <div className="flex flex-col gap-3 p-4 h-full overflow-y-auto" style={{ background: '#04060E' }}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowAdd(true)} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-purple-500/15 border border-purple-500/30 text-purple-300 font-orbitron text-[9px] font-bold tracking-wider hover:bg-purple-500/25 transition-all active:scale-[0.98]">
            <Plus className="w-3 h-3" /> ADD PLAYER
          </button>
          <button onClick={handleExport} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-blue-500/15 border border-blue-500/30 text-blue-300 font-orbitron text-[9px] font-bold tracking-wider hover:bg-blue-500/25 transition-all active:scale-[0.98]">
            <Download className="w-3 h-3" /> EXPORT
          </button>
          <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 rounded bg-white/5 border border-white/[0.08] text-white/60 font-orbitron text-[9px] font-bold tracking-wider hover:bg-white/10 transition-all active:scale-[0.98]">
          ...
          <div className="flex rounded border border-white/[0.08] overflow-hidden">
            <button onClick={() => setViewMode('grid')} className={`p-1.5 ${viewMode === 'grid' ? 'bg-purple-500/20 text-purple-300' : 'text-white/40 hover:bg-white/[0.04]'}`}><Grid3x3 className="w-3 h-3" /></button>
            <button onClick={() => setViewMode('roster')} className={`p-1.5 ${viewMode === 'roster' ? 'bg-purple-500/20 text-purple-300' : 'text-white/40 hover:bg-white/[0.04]'}`}><List className="w-3 h-3" /></button>
          </div>
        </div>
      </div>
      ...
      {/* FILTERS */}
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1.5 flex-1 px-3 py-2 rounded-lg border border-white/[0.08]" style={{ background: '#131127' }}>
        ...
        <select value={teamFilter} onChange={e => setTeamFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-white/[0.08] text-[10px] font-orbitron font-bold text-white outline-none cursor-pointer" style={{ background: '#131127' }}>
        ...
        <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="px-3 py-2 rounded-lg border border-white/[0.08] text-[10px] font-orbitron font-bold text-white outline-none cursor-pointer" style={{ background: '#131127' }}>
        ...
        <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="px-3 py-2 rounded-lg border border-white/[0.08] text-[10px] font-orbitron font-bold text-white outline-none cursor-pointer" style={{ background: '#131127' }}>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="flex flex-col gap-2">
          {groupedByTeam.map(({ team, players: tPlayers }) => (
            <div key={team.id || '_unassigned'} className="rounded-lg border border-white/[0.06] overflow-hidden" style={{ background: '#131127' }}>
              <button
              ...
              {expandedTeam === (team.id || '_unassigned') && (
                <div className="border-t border-white/[0.06]">
                  {tPlayers.map(p => (
                    <div key={p.id} className="flex items-center gap-3 px-6 py-2 border-b border-white/[0.06] hover:bg-white/[0.04] transition-colors">
...
function PlayerCard({ player, team, onEdit, onEliminate }) {
  return (
    <div className="group rounded-lg border border-white/[0.06] p-3 transition-all hover:border-purple-500/30" style={{ background: '#131127' }}>
      ...
      <div className="flex items-center gap-1.5 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={onEdit} className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded bg-white/5 text-white/60 hover:bg-white/10 transition-all active:scale-[0.98] text-[9px] font-bold">
          <Edit2 className="w-2.5 h-2.5" /> EDIT
        </button>
        <button onClick={onEliminate} className="flex-1 flex items-center justify-center gap-1 py-1.5 rounded transition-all active:scale-[0.98] text-[9px] font-bold" style={{
...
function StatBox({ label, value, color }) {
  return (
    <div className="rounded-lg border border-white/[0.06] p-2.5 text-center" style={{ background: '#0F1127' }}>
...
function Modal({ title, onClose, children }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
      <div className="w-full max-w-md max-h-[80vh] overflow-y-auto rounded-xl border border-white/[0.08] p-5" style={{ background: '#131127' }} onClick={e => e.stopPropagation()}>
