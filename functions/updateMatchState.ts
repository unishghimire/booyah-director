...
Deno.serve(async (req) => {
  try {
    ...
    if (!match_id || !state || !validStates.includes(state)) {
      return new Response(JSON.stringify({ error: 'match_id and valid state required' }), {
        status: 400,
    ...
    await base44.entities.Match.update(match_id, updateData);

    // Auto-switch overlay screen based on state
    ...
    if (screenMap[state]) {
      ...
      if (overlayStates.length > 0) {
        await base44.entities.OverlayState.update(overlayStates[0].id, {
          current_screen: screenMap[state],
          last_updated_at: new Date().toISOString()
        });
      }
    }

    return new Response(JSON.stringify({ success: true, match_id, state, overlay_screen: screenMap[state] || null }), {
      headers: { 'Content-Type': 'application/json' }
    });
  } catch (err) {
