...
Deno.serve(async (req) => {
  try {
    ...
    // Check team count
    const existingTeams = await base44.entities.Team.filter({ tournament_id });
    if (existingTeams.length >= 12) return new Response(JSON.stringify({ error: 'Maximum 12 teams allowed per tournament' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    ...
    return new Response(JSON.stringify({ success: true, team: { id: team.id, ...team.data }, players: createdPlayers }), {
