...
export default function Bootstrap() {
  ...
  return (
    <div className="min-h-screen bg-[#060912] flex items-center justify-center relative overflow-hidden">
      <div style={{ position:'absolute', inset:0, backgroundImage:'linear-gradient(rgba(124,58,237,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(0,212,255,0.03) 1px, transparent 1px)', backgroundSize:'80px 80px' }} />
      <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 1000px 600px at 50% 40%, rgba(124,58,237,0.08), transparent)' }} />

      <div className="relative z-10 w-full max-w-sm px-4 space-y-5">
        {/* Logo */}
        <div className="text-center">
          <div className="inline-flex w-14 h-14 rounded-2xl bg-[#7C3AED]/15 border border-[#7C3AED]/40 items-center justify-center mb-3">
            <Key className="w-7 h-7 text-[#7C3AED]" />
          </div>
          <h1 className="font-orbitron text-lg font-black text-white tracking-wider">SUPER ADMIN SETUP</h1>
          <p className="font-orbitron text-[9px] text-[#3B82F6] tracking-widest mt-1">FIRST-TIME BOOTSTRAP</p>
        </div>

        <div className="rounded-xl border border-[#7C3AED]/25 bg-[#0a0e1a] p-5 space-y-1 text-[9px] font-orbitron text-gray-500 leading-relaxed tracking-wide">
          <p className="text-[#7C3AED] font-black">⚠️ ONE-TIME SETUP ONLY</p>
        ...
        {envStatus && (
          <div className="mt-2 pt-2 border-t border-white/5 space-y-1">
            <p className="text-[#3B82F6] font-black text-[9px] tracking-wider">SERVER ENV STATUS:</p>
            {[
        ...
        <form onSubmit={handleBootstrap} className="rounded-2xl border border-white/8 bg-[#0a0e1a] p-5 space-y-4">
          ...
          <div>
            <label className="font-orbitron text-[9px] text-gray-500 tracking-wider block mb-1.5">
              <Shield className="inline w-3 h-3 mr-1 text-[#7C3AED]" />
              SUPER ADMIN SECRET
            </label>
            <input
          ...
          <div>
            <label className="font-orbitron text-[9px] text-gray-500 tracking-wider block mb-1.5">
              <UserPlus className="inline w-3 h-3 mr-1 text-[#3B82F6]" />
              ADMIN EMAIL
            </label>
            <input
